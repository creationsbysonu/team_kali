"""
Seed Tenants Management Command

Creates sample tenants (ministries) for development.
"""
...
class Command(BaseCommand):
    help = 'Seed sample tenants (ministries) for development'

    def add_arguments(self, parser):
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing tenants before seeding',
        )
    ...
    def handle(self, *args, **options):
        ...
        for ministry_data in ministries:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'  ✓ Created: {tenant.name}')
                )
            else:
                self.stdout.write(f'  - Exists: {tenant.name}')

        # Create admin users for tenants if super admin exists
